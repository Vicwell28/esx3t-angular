...
@Component({
  ...
  styleUrls: ['./df-sale-detail.component.css']
})
export class DialogsFormSaleDetailComponent {
  ...
  ngOnInit(): void {  
    ...
    this.title = this.isEdit ? 'Actualizar detalles de venta ' : 'Agregar detalles de venta ';
  ...
  onCheckboxChange(event: any, product_id: number) {
    ...
    if (event.target.checked) {
      ...
      const psIndex = this.productsSelected.findIndex(x => x.product_id === product_id);
