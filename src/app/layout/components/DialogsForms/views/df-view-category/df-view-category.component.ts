...
export class DialogsFormViewCategoryComponent implements OnInit {
  constructor(
    public dialogRef: MatDialogRef<DialogsFormViewCategoryComponent>,
  ...
  ngOnInit(): void {
    ...
    this.title = this.isEdit ? 'Actualizar Categoría' : 'Agregar Categoría';
    ...
    request.subscribe({
      next: (response) => {
        ...
        const successMessage = this.isEdit
          ? 'Se actualizó la categoría correctamente.'
