...
import { DialogsFormBrancheComponent } from 'src/app/layout/components/DialogsForms/branche/df-branche/df-branche.component';

@Component({
  selector: 'app-branches',
  templateUrl: './branches.component.html',
  styleUrls: ['./branches.component.css', '../style-table.css'],
})
export class BranchesComponent implements AfterViewInit {
  // Propiedades del componente
  displayedColumns: string[] = ['id', 'name', 'address', 'postal_code', 'citie_id', 'status', 'options'];
  dataSource: any = [];
  dialogRef?: MatDialogRef<DialogsFormBrancheComponent>;
  isLoadingPDF = false;
  ...
  openDialog(
    ...
    this.dialogRef = this.dialog.open(DialogsFormBrancheComponent, {
      width: '50%',
      height: '90%',
  ...
  getAllBranches(): void {
    ...
    this.BrancheService.indexBranches().subscribe({
      next: (value) => {
        ...
        this.dataSource = new MatTableDataSource(value.data as IBranch[]);
  ...
  changeStatus(id: number): void {
    ...
    this.BrancheService.destroyBranches(id).subscribe({
      next: (value) => {
        console.log(value);
      },
      complete: () => {
