...
import { DialogsFormProductBrancheComponent } from 'src/app/layout/components/DialogsForms/product/df-product-branche/df-product-branche.component';

@Component({
  selector: 'app-product-branches',
  templateUrl: './product-branches.component.html',
  styleUrls: ['./product-branches.component.css', '../../../style-table.css']
})
export class ProductBranchesComponent implements AfterViewInit {
  displayedColumns: string[] = ['id', 'stock', 'product_id' ,'branche_id', 'status', 'options'];
  dataSource: any = [];
  dialogRef?: MatDialogRef<DialogsFormProductBrancheComponent>;
  isLoadingPDF = false;
  ...
  constructor(private dialog: MatDialog, private ProductService:ProductBrancheService) {}
   ...
   openDialog(
    ...
    this.dialogRef = this.dialog.open(DialogsFormProductBrancheComponent, {
      width: '50%',
      height: '90%',
  ...
  getAllProducts(): void {
    ...
    this.ProductService.indexProduct().subscribe({
      next: (value) => {
        ...
        this.dataSource = new MatTableDataSource(value.data as IProductBranch[]);
