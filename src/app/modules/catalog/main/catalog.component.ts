...
import { errorDialog, miniSuccesDialog, successDialog } from 'src/app/layout/components/alert';
...
export class CatalogComponent {
  ...
  onKeyUp() {
    if (this.searchText) {
      ...
      this.results = this.ProductsList.filter(item => {
        return item.name.toLowerCase().includes(this.searchText.toLowerCase()) ||
          item.pricedescount.toLowerCase().includes(this.searchText.toLowerCase())
      });
    } else {
      this.results = this.ProductsList;
    }
